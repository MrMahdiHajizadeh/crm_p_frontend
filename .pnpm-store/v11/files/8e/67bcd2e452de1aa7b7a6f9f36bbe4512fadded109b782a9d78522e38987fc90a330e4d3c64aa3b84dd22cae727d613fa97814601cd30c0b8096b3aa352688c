...
function getEnvVar(env, varName) {
  return typeof env === 'object' && env !== null && varName in env && typeof (env )[varName] === 'string'
    ? ((env )[varName] )
    : undefined;
}
...
function getFinalOptions(userOptions = {}, env) {
  ...
  return {
    ...
    debug: userOptions.debug ?? core.envToBool(getEnvVar(env, 'SENTRY_DEBUG')),
    tunnel: userOptions.tunnel ?? getEnvVar(env, 'SENTRY_TUNNEL'),
  };
}

exports.getFinalOptions = getFinalOptions;
