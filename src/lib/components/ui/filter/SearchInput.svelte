...
<div class="relative inline-flex items-center {className || ''}" {...restProps}>
  ...
  <input
    ...
    {placeholder}
    class="h-7 w-full rounded-[var(--r-sm)] border border-[color:var(--border-faint)] bg-[color:var(--bg-elevated)] py-0 ps-8 pe-7 text-[13px] text-[color:var(--text)] placeholder:text-[color:var(--text-subtle)] outline-none transition-colors focus:border-[color:var(--violet)]/50 focus:ring-1 focus:ring-[color:var(--violet)]/30"
  />
  {#if inputValue}
    <button
      type="button"
